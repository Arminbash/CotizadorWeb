//------------------------------------------------------------------------------
// <auto-generated>
//     Este código se generó a partir de una plantilla.
//
//     Los cambios manuales en este archivo pueden causar un comportamiento inesperado de la aplicación.
//     Los cambios manuales en este archivo se sobrescribirán si se regenera el código.
// </auto-generated>
//------------------------------------------------------------------------------

namespace CotizadorWeb.Domain.Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class DevolucionFactura
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public DevolucionFactura()
        {
            this.DetalleDevolucionFactura = new HashSet<DetalleDevolucionFactura>();
        }
    
        public int IdDevolucionFactura { get; set; }
        public Nullable<System.DateTime> Fecha { get; set; }
        public Nullable<int> IdEncargadoDevolucion { get; set; }
        public string Observacion { get; set; }
        public Nullable<int> IdFactura { get; set; }
        public Nullable<int> IdSerie { get; set; }
        public string Consecutivo { get; set; }
        public string SerieConsecutivo { get; set; }
        public string TipoDevolucion { get; set; }
    
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<DetalleDevolucionFactura> DetalleDevolucionFactura { get; set; }
        public virtual Factura Factura { get; set; }
    }
}
